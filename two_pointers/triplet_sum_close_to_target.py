...
'''
# my answer
def triplet_sum_closest_to_target(arr, target):
	closest_sum = target 
	closest_triplet = []
	arr.sort()

	def search_pairs(array, current_number, left):
		right=len(array)-1
		while left < right:
			current_sum = current_number + array[left] + array[right]
			if current_sum > closest_sum:
				closest_sum = current_sum
				closest_triplet = [current_number, array[left], array[right]]
	...
	for i in range(len(arr)-1):
		search_pairs(arr, arr[i], i+1, closest_triplet, closest_sum)
	return closest_sum
