...
def longest_substring_with_k_distinct(arr, k):
  ...
  for right in range(len(arr)):
    if arr[right] in seen:
      ...
      seen[arr[right]] = 1
    longest += 1
    if (len(seen) == 0) or (len(seen) == 1 and arr[right] in seen):
      # start keeping track of the first letter
      first_letter = arr[right]
      index_first_letter = right
    if len(seen) > k:
      # need to remove some
      left = index_first_letter+1
      del seen[first_letter]
      first_letter = arr[right]
      index_first_letter = right
      print("longest ", longest)
      longest = longest - index_first_letter + 1
